...
fixation_times = Dict{Tuple{Int64, Int64, Float64, Float64, Float64},Float64}()
c_freqs = Dict{Tuple{Int64, Int64, Float64, Float64, Float64},Float64}()
p_freqs = Dict{Tuple{Int64, Int64, Float64, Float64, Float64},Float64}()
...
strategies = sort(unique(runs[:strategies]))

param_combs = collect(Base.product(k_vals, strategies, z_vals, d_vals, r_vals))

for (pi, param_comb) in enumerate(param_combs)
    k, strats, z, d, r = param_comb
    #println("$param_comb")
    type_freqs[param_comb] = zeros(Float64, 4)
    c_freqs[param_comb] = 0.0
    p_freqs[param_comb] = 0.0
    fixation_times[param_comb] = 0.0
    tmp_runs = runs[(runs[:k] .== k) .& (runs[:strategies] .== strats) .& (runs[:z] .== z) .& (runs[:d] .== d) .& (runs[:r] .== r), :]
...
for (ki, k) in enumerate(k_vals)
    for (si, strats) in enumerate(strategies)
        fig, axs = plt.subplots(4, length(d_vals), figsize=(12,12),
            ...
            for (di, d) in enumerate(d_vals)
                ...
                for (zi, z) in enumerate(z_vals)
                    for (ri, r) in enumerate(r_vals)
                        freqs_im[ri, zi, :] += type_freqs[k, strats, z, d, r]
                    end
                end
                for i in 1:4
                    ax = axs[i,di]
            ...
            fig.suptitle("payback, strats = $strats, k = $k")
        ...
        for (di, d) in enumerate(d_vals)
            ...
            for (zi, z) in enumerate(z_vals)
                for (ri, r) in enumerate(r_vals)
                    c_freq_im[ri, zi] = c_freqs[k, strats, z, d, r]
                    p_freq_im[ri, zi] = p_freqs[k, strats, z, d, r]
                end
            end
            ax = axs[1,di]
        ...
        for (di, d) in enumerate(d_vals)
            ...
            for (zi, z) in enumerate(z_vals)
                for (ri, r) in enumerate(r_vals)
                    generations[ri, zi] = fixation_times[k, strats, z, d, r]
                end
            end
            ax = axs[di]
